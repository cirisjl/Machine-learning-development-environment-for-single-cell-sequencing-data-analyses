...
const TaskTable = () => {
    ...
    const fetchTasks = async (currentPage) => {
        const response = await fetch(`${NODE_API_URL}/getJobs?authToken=${jwtToken}&page=${currentPage}`);
                        ...
                        <tbody>
                            {tasks.map((task) => (
                                <TableRow key={task.job_id}>
                                    ...
                                    <TableCell>
                                        {new Intl.DateTimeFormat('en-US', timestampScheme).format(new Date(moment.utc(task['Completed on']).local()))}
                                    </TableCell>
