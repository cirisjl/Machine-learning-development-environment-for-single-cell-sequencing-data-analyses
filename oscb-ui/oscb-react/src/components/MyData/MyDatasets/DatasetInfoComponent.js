  ...
  const handleChange = (processId) => async (event, isExpanded) => {
    ...
    if (isExpanded && !details[processId]) {
      ...
      try {
        ...
        const response = await fetch(`${NODE_API_URL}/getPreProcessResult`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
          body: JSON.stringify({
            processId: processId, // Pass the processId dynamically
          }),
        });
        ...
        setDetails((prevDetails) => ({
          ...prevDetails,
          [processId]: data, // Store fetched data for the corresponding process_id
        }));
      } catch (error) {
