            ...
            <div align="center">
              {
                toolResultsFromMongo.map((result, index) => (
                  <React.Fragment key={index}>
                    ...
                    {(() => {
                      const jsonData = {
                        'celltypist_label': ['Hepatocytes', 'Hepatocytes', 'Endothelial cells', 'T cells'],
                        'celltypist_ref_label': ['hepatocyte', 'hepatocyte', 'hepatocyte', 'hepatocyte'],
                        'SingleR_main': ['Hepatocytes', 'Hepatocytes', 'Endothelial cells', 'Hepatocytes'],
                        'SingleR_fine': ['Hepatocytes', 'Hepatocytes', 'Endothelial cells', 'NPCs'],
                        'SingleR_user_ref': ['hepatocyte', 'hepatocyte', 'endothelial cell of hepatic sinusoid', 'hepatocyte'],
                        'scANVI_predicted': ['hepatocyte', 'hepatocyte', 'endothelial cell of hepatic sinusoid', 'hepatocyte'],
                        'celltypist_score': [0.43681206651165355, 0.6427823829263568, 0.4713544012637107, 0.036982124629491846],
                        'celltypist_ref_score': [0.9913741076519281, 0.9931260291572482, 0.4866464701676811, 0.9316446049079701],
                        'scANVI_transfer_score': [0.8981132507324219, 0.9146138429641724, 0.6969335079193115, 0.5827721357345581],
                        'cluster': ['0', '1', '2', '3']
                      };

                      // Transform column-oriented data to row-oriented
                      const rows = jsonData.cluster.map((_, index) => {
                        const row = {};
                        Object.keys(jsonData).forEach(key => {
                          row[key] = jsonData[key][index];
                        });
                        return row;
                      });

                      const clusters = jsonData.cluster; // Available clusters

                      return (
                        <Grid item xs={12} sx={{ marginTop: 4 }}>
                          <Card raised sx={cardStyle}>
                            <CardHeader title="Cluster Analysis" />
                            <CardContent sx={cardContentStyle}>
                                ...
                                <Table sx={{ minWidth: 650 }} aria-label="simple table">
                                  <TableHead>
                                    <TableRow>
                                      ...
                                      <TableCell><strong>Cluster Operation</strong></TableCell>
                                      <TableCell><strong>Manual Annotation</strong></TableCell>
                                    </TableRow>
                                  </TableHead>
                                  <TableBody>
                                        ...
                                        <TableCell>
                                          ...
                                          >
                                            ...
                                            {clusters.map(cluster => (
                                              <MenuItem key={cluster} value={`Merge into Cluster ${cluster}`}>
                                                Merge to Cluster {cluster}
                                              </MenuItem>
                                            ))}
                                          </Select>
                                        </TableCell>
