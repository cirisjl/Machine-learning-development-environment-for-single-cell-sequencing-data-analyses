...
Workflows are customized combinations of different tools. For example, you can run multiple normalization methods simultaneously to generate clustering results.

Currently, we provide Clustering, Integration, and Annotation workflows, with more to be added in the future.
...
All workflows begin with **Quality Control (QC)**. QC methods include `Bioconductor` [^1], `Scanpy` [^2], `Seurat` [^3], and `Dropkick` [^4]. Parameters include `min genes`, `max genes`, `min cells`, `Highly Variable Genes (HVG)`, `expected doublet rate`, and `regress cell cycle`. This task removes low-quality cells, annotates doublets and outliers, regresses cell cycle effects, and computes HVGs.

> [!TIP] 
> You may select **ONLY ONE** QC method at the same time.
> 
...
> [!TIP]
> You may select multiple Normalization methods at the same time.
>
...
> | --------------------- | ----------------------------------------------------------------------------------------------- | ----------------------------------------------------------- | ---------------------------- |
> | **LogCPM**            | Scales raw counts per million and applies log transformation.                                   | Simple, fast, and widely supported normalization.           | `edgeR` [^7], `Scanpy`  [^2]           |
> | **LogCP10K**          | Scales counts to 10,000 per cell (Seurat style).                                                | Standard in Seurat pipelines.                               | `Seurat::NormalizeData`      |
> | **sctransform**       | Uses regularized negative binomial regression to remove technical noise and stabilize variance. | High-quality, variance-stabilized normalization.            | `Seurat (SCTransform)` [^3]       |
> | **DESeq2**            | Uses the median ratio method to estimate size factors and correct library size differences.     | Bulk-like scRNA data or pseudobulk differential expression. | `DESeq2` [^6]                     |
...
> | **TMM**               | Trims extreme log-fold changes to correct composition bias.                                     | Datasets with composition bias (mostly bulk RNA-seq).       | `edgeR` [^7]                      |
> | **RLE**               | Centers each gene’s expression around its median across samples.                                | When most genes have similar expression levels.             | `DESeq2` [^6]                    |
...
> | **Pearson Residuals** | Uses residuals from a negative binomial model for variance stabilization.                       | Advanced modeling (e.g., `scTransform`, `scVI`).            | `Scanpy` [^2], `Seurat` [^3]           |

> [!CAUTION]  
> Avoid using `FPKM/TPM` for **UMI-based scRNA-seq** — they are designed for bulk RNA-seq and don’t handle dropout sparsity well.

**Imputation** methods include `MAGIC` [^9] and `SAVER` [^10].
...
Integration workflow begin with QC, followed by Integration. Methods include `Seurat` [^3], `Harmony` [^11], `Liger` [^12], and `scVI` [^13]. Parameters include a `Batch Key` (e.g., "sample", "batch") and `Pseudo Replicates`. If samples are pooled, indicate the number of individual samples per batch to generate `pseudo-replicates`.
...
Annotation workflow begin with QC, followed by Integration and then Annotation. Annotation methods include `CellTypist` [^14], `scANVI` [^13], and `SingleR` [^15]. Parameters include `species`, `CellTypist model` (if using `CellTypist`), `SingleR reference` (if using `SingleR`), and `Reference dataset` for `CellTypist`, `SingleR`, and `scANVI` (if applicable, with `Cell Type Labels` included).
...
>
> A workflow may consist of multiple processes. You can expand each process to view detailed results.
...
## References
[^1]: Huber W, Carey VJ, Gentleman R, et al (2015) Orchestrating high-throughput genomic analysis with Bioconductor. Nat Methods 12:115–121. https://doi.org/10.1038/nmeth.3252
[^2]: Wolf FA, Angerer P, Theis FJ (2018) SCANPY: large-scale single-cell gene expression data analysis. Genome Biol 19:15. https://doi.org/10.1186/s13059-017-1382-0
[^3]: Satija R, Farrell JA, Gennert D, et al (2015) Spatial reconstruction of single-cell gene expression data. Nat Biotechnol 33:495–502. https://doi.org/10.1038/nbt.3192
[^4]: Heiser CN, Wang VM, Chen B, et al (2021) Automated quality control and cell identification of droplet-based single-cell data using dropkick. Genome Res 31:1742–1752. https://doi.org/10.1101/gr.271908.120
[^5]: Hafemeister C, Satija R (2020) Analyzing scRNA-seq data with the Sctransform and offset models
[^6]: Love MI, Huber W, Anders S (2014) Moderated estimation of fold change and dispersion for RNA-seq data with DESeq2. Genome Biol 15:550. https://doi.org/10.1186/s13059-014-0550-8
[^7]: Hafemeister C, Satija R (2020) Analyzing scRNA-seq data with the Sctransform and offset models
[^8]: Durinck S, Moreau Y, Kasprzyk A, et al (2005) BioMart and Bioconductor: a powerful link between biological databases and microarray data analysis. Bioinformatics 21:3439–3440. https://doi.org/10.1093/bioinformatics/bti525
[^9]: Dijk D van, Sharma R, Nainys J, et al (2018) Recovering Gene Interactions from Single-Cell Data Using Data Diffusion. Cell 174:716-729.e27. https://doi.org/10.1016/j.cell.2018.05.061
[^10]: Huang M, Wang J, Torre E, et al (2018) SAVER: gene expression recovery for single-cell RNA sequencing. Nat Methods 15:539–542. https://doi.org/10.1038/s41592-018-0033-z
[^11]: Korsunsky I, Millard N, Fan J, et al (2019) Fast, sensitive and accurate integration of single-cell data with Harmony. Nat Methods 16:1289–1296. https://doi.org/10.1038/s41592-019-0619-0
[^12]: Jointly defining cell types from multiple single-cell datasets using LIGER | Nature Protocols. https://www.nature.com/articles/s41596-020-0391-8. Accessed 5 Nov 2024
[^13]: Gayoso A, Lopez R, Xing G, et al (2022) A Python library for probabilistic analysis of single-cell omics data. Nat Biotechnol 40:163–166. https://doi.org/10.1038/s41587-021-01206-w
[^14]: Domínguez Conde C, Xu C, Jarvis LB, et al (2022) Cross-tissue immune cell analysis reveals tissue-specific features in humans. Science 376:eabl5197. https://doi.org/10.1126/science.abl5197
[^15]: Aran D, Looney AP, Liu L, et al (2019) Reference-based analysis of lung single-cell sequencing reveals a transitional profibrotic macrophage. Nat Immunol 20:163–172. https://doi.org/10.1038/s41590-018-0276-y