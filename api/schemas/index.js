                ...
                connection.query('INSERT INTO dataset (title, n_cells, reference, summary, user_id) VALUES (?, ?, ?, ?, ?)', [title, n_cells, reference, summary, userId], function (err, datasetResult) {
                    if (err) {
                        ...
                        for (let file of files) {
                            ...
                            connection.query('INSERT INTO file (file_loc, datasetId) VALUES (?, ?)', [file, datasetId]);
                        }
    ...
    pool.getConnection(function (err, connection) {
        ...
        connection.beginTransaction(function (err) {
            ...
            connection.query('SELECT user_id FROM users WHERE username = ? LIMIT 1', [username], function (err, userRows) {
                ...
                connection.query('SELECT datasetId FROM dataset WHERE user_id = ? and title = ? LIMIT 1', [userId, title], function (err, datasetRows) {
                    if (err) {
                        connection.rollback(function () {
                            connection.release();
                        });
                    ...
                    const datasetId = datasetRows[0].datasetId;
                    ...
                    connection.query('UPDATE dataset SET n_cells=?, reference=?, summary=? WHERE datasetId=?', [n_cells, reference, summary, datasetId], function (err, datasetResult) {
                        if (err) {
                            connection.rollback(function () {
                                connection.release();
                            });
                        ...
                        for (const file of deleteList) {
                            connection.query('DELETE FROM file WHERE file_loc=? AND datasetId=?', [file, datasetId]);
                        }
...
//                 connection.query('SELECT datasetId FROM dataset WHERE user_id = ? and title = ? LIMIT 1', [userId, dataset], function (err, datasetRows) {
//                     if (err) {
...
//                     connection.query('delete FROM file WHERE datasetId=?', [datasetId], function (err, datasetResult) {
//                         if (err) {
...
//                         }
//                         connection.query('DELETE FROM dataset where datasetId=?', [datasetId]);
...
app.post('/node/renameFile', async (req, res) => {
    ...
    pool.query(`SELECT f.file_loc FROM oscb.file f JOIN oscb.dataset d ON f.datasetId = d.datasetId JOIN oscb.users u ON d.user_id = u.user_id WHERE u.username = '${uname}' AND f.file_loc LIKE '${oldName}%';`, (err, results) => {
        if (err) {
            console.error(err);
...
app.delete('/node/deleteFiles', async (req, res) => {
    ...
    if (fileList && Array.isArray(fileList)) {
        ...
        for (const file of fileList) {
            try {
                const [rows, fields] = await pool.promise().execute(`SELECT f.file_loc FROM oscb.file f JOIN oscb.dataset d ON f.datasetId = d.datasetId JOIN oscb.users u ON d.user_id = u.user_id WHERE u.username = '${uname}' AND f.file_loc like '${file.replace("'", "''")}%';`);
...
app.get('/node/preview/datasets', (req, res) => {
    ...
    pool.query(userQuery, (err, userResult) => {
        ...
        if (userResult.length === 0) {
            ...
            const datasetsQuery = `
          SELECT dataset.datasetId, dataset.title, dataset.n_cells, dataset.reference, dataset.summary, file.file_id, file.file_loc, SUBSTRING_INDEX(SUBSTRING_INDEX(file.file_loc, '/', 2), '/', -1) AS direc
          FROM dataset
          JOIN file ON dataset.datasetId = file.datasetId
        ...
        `;
            ...
            pool.query(datasetsQuery, (err, datasetsResult) => {
                ...
                datasetsResult.forEach(row => {
                    const { datasetId, title, n_cells, reference, summary, file_id, file_loc, direc } = row;
