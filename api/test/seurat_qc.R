...
RunSeuratQC <- function(input, output, adata_path=NULL, assay='RNA', min_genes=200, max_genes=0, min_UMI_count=0, max_UMI_count=0, percent_mt_max=5, percent_rb_min=0, resolution=0.5, dims=10, n_pcs=30, doublet_rate=0.075, regress_cell_cycle=FALSE) {
    ...
    if(!is.null(srat)){
        ...
        if(default_assay==assay){
            ...
            # srat <- RunPCA(srat, features = VariableFeatures(srat), ndims.print = 6:10, nfeatures.print = 10)
            tryCatch({
                print(id, "Running PCA.")
                srat <- RunPCA(srat, npcs=n_pcs, features=VariableFeatures(object=srat))
            }, error = function(e) {
                print(paste0("An error occurred when running t-SNE/UMAP, please try a different parameter n_pcs later, skipped: ", e$message))
            }) 
            ...
            tryCatch({
                ...
                print(paste0("An error occurred when running t-SNE/UMAP, please try a different parameter n_neighbors later, skipped: ", e$message))
            }) 
