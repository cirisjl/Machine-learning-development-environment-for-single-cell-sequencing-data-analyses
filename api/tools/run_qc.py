...
def run_qc(task_id, dataset, input,userID, output, methods, idtype='SYMBOL', colour_by='NULL', shape_by_1='NULL', shape_by_2='NULL', default_assay='RNA', show_error=True):
    ...
    if "SCANPY" in methods or "DROPKICK" in methods:
        ...
        if "SCANPY" in methods:
            try:
                ...
                print("Scanpy QC is failed. The dataset might be normalized already, so all the counts are less than the cutoffs.")
        ...
        if "DROPKICK" in methods:
            try:
                ...
                print("Dropkick QC is failed. The dataset might be normalized already, so all the counts are less than the cutoffs.")
                if show_error: print(e)
    ...
    if "BIOCONDUCTOR" in methods:
        try:
            ...
            print("Bioconductor QC is failed. The dataset might be normalized already, so all the counts are less than the cutoffs.")
            if show_error: print(e)
    ...
    if "SEURAT" in methods:
        try:
            ...
            print("Seurat QC is failed. The dataset might be normalized already, so all the counts are less than the cutoffs.")
            if show_error: print(e)
