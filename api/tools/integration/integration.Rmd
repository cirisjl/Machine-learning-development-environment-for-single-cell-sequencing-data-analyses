...
# if(!is.null(fig_path) && !dir.exists(fig_path)) dir.create(fig_path, recursive = TRUE)
...
if("SEURAT" %in% methods) {
  tryCatch({
    ...
    p_seurat <- DimPlot(srats.integrated, reduction = "umap", split.by = batch_key) + NoLegend()
    print(p_seurat)
    # if(!is.null(fig_path)) ggsave(paste0(fig_path, "/Seurat_Integration.png"), plot = p_seurat, width = 6, height = 4, units = "in", dpi = 300)
    ...
    adata <- SeuratToAnndata(srats.integrated, out_file=params$adata_path, main_layer="scale.data", transfer_layers="data", assay="integrated", transfer_assay="RNA", transfer_assay_obsm="counts")
...
if("HARMONY" %in% methods) {
  tryCatch({
    ...
    p_harmony <- DimPlot(srat.harmony, reduction = "umap", group.by = batch_key, pt.size = .1, split.by = batch_key) + NoLegend()
    print(p_harmony)
    # if(!is.null(fig_path)) ggsave(paste0(fig_path, "/Harmony_Integration.png"), plot = p_harmony, width = 6, height = 4, units = "in", dpi = 300)
...
if("LIGER" %in% methods) {
  tryCatch({
    ...
    p_liger <- DimPlot(srat.liger, reduction = "umap", group.by = batch_key, pt.size = .1, split.by = batch_key) + NoLegend()
    print(p_liger)
    # if(!is.null(fig_path)) ggsave(paste0(fig_path, "/LIGER_Integration.png"), plot = p_liger, width = 6, height = 4, units = "in", dpi = 300)
