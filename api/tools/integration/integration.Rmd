...
date: "`r Sys.Date()`"
params:
  unique_id: integration # unique_id for live log, which could be task_id, user_id or process_id
...
if("SEURAT" %in% methods) {
  tryCatch({
    ...
    adata <- SeuratToAnndata(srats.integrated, params$adata_path)
    rm(adata)
...
if("HARMONY" %in% methods) {
  tryCatch({
    ...
    adata <- SeuratToAnndata(srat.harmony, params$adata_path)
    rm(adata)
...
if("LIGER" %in% methods) {
  tryCatch({
    ...
    adata <- SeuratToAnndata(srat.liger, params$adata_path)
    rm(adata)
