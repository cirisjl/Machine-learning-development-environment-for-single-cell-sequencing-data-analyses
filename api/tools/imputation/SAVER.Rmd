...
params:
  ...
  ncores: 12 # Number of workers for SAVER imputation
  show_code: FALSE
  show_warning: TRUE
  show_message: TRUE
  show_error: TRUE
...
```
## SAVER imputation
```{r SAVER imputation, include=TRUE, echo=params$show_code, warning=params$show_warning, message=params$show_message}
library(SAVER)
...
if(!'SAVER_imputed' %in% names(adata$layers)){
    tryCatch(
        {
            counts.saver <- NULL
            ...
            adata$layers["SAVER_imputed"] <- t(counts.saver)
            head(adata$layers["SAVER_imputed"])
