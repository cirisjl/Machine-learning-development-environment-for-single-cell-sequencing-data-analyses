...
date: "`r Sys.Date()`"
params:
  unique_id: normalization # unique_id for live log, which could be task_id, user_id or process_id
  dataset: NA
...
# adata_path <- gsub(".h5seurat", ".h5ad", params$output)
...
```{r logCP10K, error=TRUE, include=TRUE, echo=params$show_code, warning=params$show_warning, message=params$show_message}
...
```{r logCPM, error=TRUE, include=TRUE, echo=params$show_code, warning=params$show_warning, message=params$show_message}
...
```{r SCTransform, error=TRUE, include=TRUE, echo=params$show_code, warning=params$show_warning, message=params$show_message}
if(("SCT" %in% methods) || ("SCT_V2" %in% methods)){
  ...
  if(!"SCT" %in% names(srat@assays)){
    ...
    RedisInfo(unique_id, "SCT assay already exists, skipped.")
    adata_sct <- AnnData(X = t(srat[['SCT']]@data), obs = srat@meta.data, var = srat[['SCT']]@meta.features)
    adata_sct.path <-  gsub(".h5ad", "_SCT.h5ad", adata_path)
    write_h5ad(adata_sct, adata_sct.path, compression = "gzip")
    RedisInfo(unique_id, "AnnData object for SCTransform is saved separately successfully.")
...
```{r scran, error=TRUE, include=TRUE, echo=params$show_code, warning=params$show_warning, message=params$show_message}
if("SCRAN" %in% methods){
...
```{r DESeq2, error=TRUE, include=TRUE, echo=params$show_code, warning=params$show_warning, message=params$show_message}
...
```{r edgeR, error=TRUE, include=TRUE, echo=params$show_code, warning=params$show_warning, message=params$show_message}
if("TMM" %in% methods || "RLE" %in% methods || "UQ" %in% methods || "UPPERQUARTILE" %in% methods){
  ...
  if("UQ" %in% methods || "UPPERQUARTILE" %in% methods){
    if(!"upperquartile" %in% names(srat@assays)){
      tryCatch({
        RedisInfo(unique_id, "Running upperquartile normalization.")
...
```{r biomaRt, error=TRUE, include=TRUE, echo=params$show_code, warning=params$show_warning, message=params$show_message}
...
if("TPM" %in% methods || "FPKM" %in% methods){
  tryCatch({
    ...
    if(params$show_umap) seurat_cluster_umap(srat, assay = 'FPKM', title = "FPKM normalization")
      ...
      RedisWarning(unique_id, paste0("An error happened during TPM/FPKM normalization, skipped: ", e$message))
  })
}
...
```{r scanpy, error=TRUE, include=TRUE, echo=params$show_code, warning=params$show_warning, message=params$show_message}
