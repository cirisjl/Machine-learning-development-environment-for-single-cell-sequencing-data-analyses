...
if(("SCTRANSFORM" %in% methods) || ("SCTRANSFORM_V2" %in% methods) || ("SCT" %in% methods) || ("SCT_V2" %in% methods)){
  ...
  if(!"SCTRANSFORM" %in% names(srat@assays)){
    tryCatch({
      library(sctransform)
      if("SCTRANSFORM"  %in% methods){
      ...
      adata_sct <- SeuratToAnndata(srat, out_file=adata_sct.path, assay="SCTransform")
  ...
  knitr::kable(head(srat[['SCTransform']]@data), align='cccc', format = "html", caption = 'SCTransform')
  if(params$show_umap) seurat_cluster_umap(srat, assay = 'SCTransform', title = "SCTransform normalization")
} else {
  print("Not selected by user, skipped.")
}
```
