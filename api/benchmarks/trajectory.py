...
def trajectory_task(adata_path, label, benchmarksId, datasetId, job_id, task_type='Trajectory', bm_traj='benchmark_traj'):
    ...
    metrics = ["Graph Edit Distance", "Graph Kernel Score", "Jaccard Similarity Coefficient", "Tree Edit Distance"]
    ...
    try:
        ...
        if scanpy_results is not None:
            ...
            # Call scanpy method
            scanpy_results = scanpy_trajectory(adata, label, benchmarksId, datasetId, task_type, bm_traj=bm_traj)
        ...
        if len(scanpy_results) > 0:
            for key, result in scanpy_results.items():
                ...
                y_values[key] = [result['Graph Edit Distance'], result['Graph Kernel Score'], result['Jaccard Similarity Coefficient'], result['Tree Edit Distance']]
                ...
                redislogger.info(job_id, f"{key}: Graph Edit Distance: {result['Graph Edit Distance']}, Graph Kernel Score: {result['Graph Kernel Score']}, Jaccard Similarity Coefficient: {result['Jaccard Similarity Coefficient']}, Tree Edit Distance: {result['Tree Edit Distance']}")
