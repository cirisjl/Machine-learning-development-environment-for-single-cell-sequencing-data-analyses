...
def create_qc_task(self, dataset, input, userID, output, methods, path_of_scrublet_calls='./scrublet_calls.tsv', show_error=True):
    results = run_qc(dataset, input,userID, output, methods, path_of_scrublet_calls='./scrublet_calls.tsv', show_error = True)
    return results
...
def create_imputation_task(self, dataset, input, output, methods, layer=None, genes=None, ncores=12, show_error=True):
    results = run_imputation(dataset, input, output, methods, layer=None, genes=None, ncores=12, show_error=True)
    return results
